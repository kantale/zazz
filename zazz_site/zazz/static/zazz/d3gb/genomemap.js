...
var y = d55.scale.linear()
    .range([0, height]);

//d55.select("body").append("div")
d55.select("#d3gbdiv").append("div")
...
if(typeof displaySelector != 'undefined'){
  //d55.select("body>div>div")
  d55.select("#d3gbdiv>div>div")
	.append("a")
	.append("button")
        .attr("disabled", true)
        .text("Go to");
  //imgInfo(d55.select("body>div>div"), function(sel){
    imgInfo(d55.select("#d3gbdiv>div>div"), function(sel){
...
//var svg = d55.select("body>div").append("svg")
var svg = d55.select("#d3gbdiv>div").append("svg")
...
".stalk {fill: #666; stroke: none}");
  ...
  var data = JSON.parse(d55.select("#data").text());

  if(typeof chromosomes == 'undefined')
    var chromosomes = JSON.parse(d55.select("#chromosomes").text());
  ...
  if (count>24){
    ...
    d55.select("svg").attr("width", width + margin.left + margin.right)
  }
...
data.chromosomes.forEach(function(chr, i){
  ...
  svg.append("text")
    ...
    var maxDom = d55.max(json,function(d){ return d[1]; }),
        maxRan = y(maxDom),
        fitScale = d55.scale.linear()
              .range([0, maxRan])
              .domain([0, maxDom]);
...
function displayButtons(){
    //d55.select("body>div>div").append("button")
    d55.select("#d3gbdiv>div>div").append("button")
	.text("SVG")
	.style("float","right")
...
var pdfWidth = width + margin.left + margin.right,
    ...
    svgDoc = d55.select("svg").node();

doc.setDrawColor(0);
doc.setLineWidth(1);

d55.selectAll(".chromosome .bands>path:not(.acen)").each(function(){
	var coords = getCoords(this, svgDoc),
	  color = d55.rgb(d55.select(this).style("fill"));
	doc.setFillColor(color.r,color.g,color.b);
	doc.rect(coords.x, coords.y, coords.x2-coords.x, coords.y2-coords.y, 'FD');
});

d55.selectAll(".chromosome path.acen").each(function(d,i){
	var coords = getCoords(this, svgDoc),
	  color = d55.rgb(d55.select(this).style("fill"));
	doc.setFillColor(color.r,color.g,color.b);
	if(i%2==0)
...
d55.selectAll(".chromosome path.mapTrack").each(function(){
  var coords = getCoords(this, svgDoc);
  doc.rect(coords.x, coords.y, coords.x2-coords.x, coords.y2-coords.y, 'F');
});

d55.selectAll(".chromosome g.brush rect.extent").each(function(){
  if(d55.select(this).style("display")!="none"){
	var coords = getCoords(this, svgDoc);
...
d55.selectAll(".track>rect:first-child").each(function(){
  var coords = getCoords(this, svgDoc),
      color = d55.rgb(d55.select(this).style("fill"));
  doc.setFillColor(color.r,color.g,color.b);
  doc.rect(coords.x, coords.y, coords.x2-coords.x, coords.y2-coords.y,'F');
...
d55.selectAll(".track>.segment>path, .track.score>path, .track>.values>path").each(function(){
  var self = d55.select(this),
      coords = getCoords(this, svgDoc),
      opc = parseFloat(self.style("fill-opacity")),
      color = d55.rgb(self.style("fill")),
...
d55.selectAll(".track.vcfmaster>.segment>rect").each(function(){
  var coords = getCoords(this, svgDoc),
      color = d55.rgb(d55.select(this).style("fill"));
  doc.setFillColor(color.r,color.g,color.b);
...
d55.selectAll("line").each(function(){
	var coords = getCoords(this, svgDoc);
	doc.line(coords.x, coords.y, coords.x2, coords.y2);
});

d55.selectAll(".x.axis path").each(function(){
	var coords = getCoords(this, svgDoc);
	doc.setDrawColor(0);
	doc.line(coords.x, coords.y2, coords.x2, coords.y2);
});

d55.selectAll("text").each(function(){
	var self = d55.select(this),
        coords = getCoords(this, svgDoc),
	t = d55.select(this.childNodes[0]).text(),
	color = d55.rgb(self.style("fill")),
	s = parseInt(self.style("font-size")),
	rotate = d55.transform(self.attr("transform")).rotate;
	doc.setTextColor(color.r,color.g,color.b);
	doc.setFontSize(s);
	doc.text(coords.x, coords.y+8, t, null, rotate?-rotate:null);
});

doc.save(d55.select("head>title").text()+".pdf");
}
