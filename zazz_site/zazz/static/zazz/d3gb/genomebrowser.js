...
function displayGenomeBrowser(){
  //body = d55.select("body");
  body = d55.select("#d3gbdiv");
  chromosomes = JSON.parse(d55.select("#chromosomes").text());
  execDB("SELECT trackid, trackname, type, color, data FROM tbl_tracks", function(res){
    dataTracks = res;
    d55.select("head").append("style")
    ...
    d55.select("#d3gbdiv").on("click", function(){
        d55.select("div.tooltip").remove();
        d55.select("select").remove();
      })
    if(/^(\?r=)([\_\.0-9a-zA-Z]+)\:[0-9]+\-[0-9]+$/.test(window.location.search)){
      displayTracks(window.location.search.substring(3));
    } else {

      setTimeout(function(){ genomemap(); }, 2000);
      //genomemap();
    }
  });
}
...
function gotoLocation(){
  var str = d55.select("input[name=r]").node().value,
      patt = new RegExp("^([\_\.0-9a-zA-Z]+)\:[0-9]+(\-[0-9]+)?$");
  if(patt.test(str)){
    displayTracks(str);
  }else{
    var option = d55.select("select[name=r]>option:first-child");
    if(!option.empty())
      displayTracks(option.property("value"));
      d55.select("select[name=r]").remove();
  }
  return false;
}
...
function searchGene(){
  ...
  if(patt.test(str)){
    execDB("SELECT * FROM aux_genes WHERE name LIKE '"+str+"%' LIMIT 100", function(gene){
      if(gene != false){
        var BCR = d55.select("input[name=r]").node().getBoundingClientRect();
        body.append("select")
              .attr("name","r")
              .attr("size", gene.length+1)
              .on("change", function(){
                displayTracks(this.value);
                d55.select(this).remove();
              })
...
function displayTooltip(infoButton, displayInfo){
  if(d55.select("div.tooltip").empty()){
    d55.event.stopPropagation();
...
function displaySelector(gChr, chrScale, selector){
  ...
  console.log('IN displaySelector');

  var brush = d55.svg.brush()
      .x(chrScale)
  ...
  if(gBrush.empty()){
    ...
    gBrush.select(".background")
      ...
      .on("mousedown", !selector ? function(){
        var id = gChr.attr("id");
        d55.selectAll(".brush .extent, .brush .resize").style("display",function(){ return id==this.parentNode.parentNode.id?null:"none"; });
      } : null)
  ...
  function brushmove() {
    ...
    if(bp>chrScale.domain()[1]){
      ...
      start = Math.round(d55.min(extent));
      end = Math.round(d55.max(extent));
      if(end-start==0){
        end = start+(bp/2);
        start = start-(bp/2);
        if(start<0){
...
function resizeHeight(sel){
  ...
  d55.select("svg").attr("height", height + margin.top + margin.bottom);
  d55.select("#mask rect").attr("height", height);
}
...
function keypressed() {
  if(d55.select("input:focus").empty()){
    if(d55.event.code=="ArrowLeft"){
          var move = parseInt((end-start)*0.1);
          start = start - move;
          end = end - move;
          displayTracks();
    }
    if(d55.event.code=="ArrowRight"){
          var move = parseInt((end-start)*0.1);
          start = start + move;
          end = end + move;
          displayTracks();
...
function getTracks(chr, start, end, callback){
  if(php){
    if(typeof xhrGetTracks != 'undefined')
      xhrGetTracks.abort();
    xhrGetTracks = d55.json('query.php')
	.header("Content-type", "application/x-www-form-urlencoded")
...
function genomebrowser(){
  ...
  nav.append("input")
     ...
     .style("width","200px")
     .on("keyup", function(){
        if (d55.event.keyCode == 13) { gotoLocation(); }
	else { searchGene(); }
      })
     .on("paste", function(){
	searchGene();
  ...
  if(dataTracks.map(function(d){ return d[2]; }).indexOf("value")!=-1){
    nav.append("button")
      ...
      .on("click", function(){
        ...
        d55.select(this).style("background",fixedBounds?"#aaa":null)
        displayTracks();
      });
  }
  ...
  nav.append("button")
    ...
    .on("click", function(){
          ...
          if(php)
	    d55.json("./query.php?r="+chr+":"+start+"-"+end, queryCallback);
          else
            get_dna([0,0,chr,start,end], queryCallback);
    });

//var svg = body.select("body>div").append("svg")
var svg = body.select("#d3gbdiv>div").append("svg")
...
".stalk {fill: #666; stroke: none}");
 ...
 buttons.append("polygon")
  ...
  var initX;
  var widthScale = d55.scale.linear()
        .domain([0,width]);
  ...
  var drag = d55.behavior.drag()
             .on('dragstart', function(){
               tracks.style("cursor","move");
               initX = d55.mouse(tracks.node())[0];
             })
             .on('drag', function(thisData) {
               var coor = d55.mouse(tracks.node())[1];
               if(coor%10==0){
             ...
             .on('dragend', function() {
               ...
               var coor = d55.mouse(tracks.node())[0],
                   x = coor - initX;
               if(Math.abs(x)>20){
...
function displayTracks(strLocation){
  ...
  var svg = d55.select("svg g.tracks");
  ...
  var chrEnd = d55.max(json, function(d){
      return d[1];
  });
...
var chrScale = d55.scale.linear()
    .range([0, width])
    .domain([0, chrEnd]);

var x = d55.scale.linear()
    .range([0, width])
    .domain([start, end]);

var scoreScale = d55.scale.linear()
    .clamp(true);

var valueScale = d55.scale.linear()
    .range([50, 0])
    .clamp(true);
...
var xAxis = d55.svg.axis()
    .scale(x)
    .orient("top")
    .tickFormat(function(d){
      if(bp>6666){
        if(d>=1e+6)
          return d55.round((d/1e+6),3)+"M";
        if(d>=1e+3)
          return d55.round((d/1e+3),3)+"K";
      }
      return d55.format(",")(d);
    });
  ...
  d55.select("span.location")
      .text("location "+chr+":"+start+"-"+end+" "+bpFormat(bp)+" ")

  d55.select("input[name=r]")
      .property("value","");
    ...
    gTrack.each(function(d,trackIndex){
      ...
      switch(type){
        case "value":
          ...
          if(fixedBounds){
            ...
            var minval = d55.min(data,function(d){ return +d[6]; }),
                maxval = d55.max(data,function(d){ return +d[6]; });
            if(minval>0)
              minval = 0;
            if(maxval<0)
              maxval = 0;
          ...
          track.selectAll(".track>g")
            ...
            .each(function(d){
              if(d=="axis"){
                d55.select(this).attr("transform", "translate("+(width-1)+")")
                .on("click", function(){
		  if(d55.select("div.tooltip").empty()){
		    d55.event.stopPropagation();
			...
			"left":(BCR.right)+"px"})
			.on("click", function(){ d55.event.stopPropagation(); })
		    tooltip.append("p")
			.append("input")
			  .attr("type","number")
			  .attr("name","max")
			...
			.append("input")
			  ...
			  .on("click", function(){
			    fixedBounds = true;
			    d55.select("button.bounds").style("background","#aaa")
			    var newScale = [d55.select("input[name=min]").property("value"),d55.select("input[name=max]").property("value")];
                    ...
                    thiz.append('rect')
                      ...
                      .style('fill',function(){
                        ...
                        if(patt.test(name) || name == "GMAF"){
                          ...
                          return d55.lab((1-n)*100,0,-40).toString();
                        }
                        if(name == "IMPACT"){
	  ...
	  smgEnter.call(addTitle);
	    ...
	    smgEnter.style('cursor','pointer').on("click", function(d){
                ...
                if(php)
	          d55.json("./query.php?r="+d[2]+":"+d[3]+"-"+d[4]+"&name="+d[5]+((d[7]=="-")?"&strand=reverse":"")+((d[8]||d[9])?"&thickStart="+d[8]+"&thickEnd="+d[9]:"")+(d[11]?"&blockCount="+d[11]+"&blockSizes="+d[12]+"&blockStarts="+d[13]:""), queryCallback);
                else
